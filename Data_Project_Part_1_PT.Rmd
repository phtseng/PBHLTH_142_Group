...
sti_data <- read_excel("/sti_data.xlsx", sheet = "2000-2014")

str(sti_data)

# Select relevant columns and filter by Chlamydia excluding unknown genders
df <-
  sti_data %>%
...
show <- Data %>% group_by(Disease, Gender) %>% mutate(Rate=as.numeric(Rate)) %>% filter(Disease=='Chlamydia') %>% summarize(TotalCount = sum(Count), TotalPop = sum(Population), TotalRate = sum(Rate)) 
...
# Plotting
Plot = ggplot(data = Data, aes(x = Proportion))
Plot = Plot + geom_histogram(aes(y = stat(density)), binwidth = 0.000055,color='black',fill='black')
